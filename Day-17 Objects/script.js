let data =[
 ...
 // console.log(sorted)
  ...
  for(i in category){
     ...
     type.style.fontSize="1em"
     type.style.fontWeight="700"
     type.className=category[i]
     type.innerText=category[i]
     type.addEventListener("click", ((i) => {
         return () => {
             display.innerHTML=""
             for (j in sorted[category[i]]) {
                 //console.log(sorted[category[i]][j])
                 let info = sorted[category[i]][j]
                 ...
                 rating.innerText=`Rating : ${info.rating.rate} `
